...
/**
 * NEW ARCHITECTURE: Two-phase analysis with detailed progress tracking
 * Phase 1: Extract transactions from all files
 * Phase 2: Categorize transactions in small, fast chunks
 */
export const analyzeStatements = async (
  ...
  console.log(`📦 Created ${extractionBatches.length} extraction batches (${EXTRACTION_BATCH_SIZE} files per batch)`);
  ...
  for (let batchIdx = 0; batchIdx < extractionBatches.length; batchIdx++) {
    ...
    console.log(`\n📦 Processing extraction batch ${batchIdx + 1}/${extractionBatches.length} (${batch.length} files)...`);
    ...
    try {
      ...
      allTransactions = allTransactions.concat(extractedTransactions);
      filesProcessed += batch.length;

      console.log(`✅ Batch ${batchIdx + 1} extracted ${extractedTransactions.length} transactions`);
      console.log(`📊 Total so far: ${allTransactions.length} transactions from ${filesProcessed} files`);
      ...
      console.error(`❌ Extraction batch ${batchIdx + 1} failed:`, error);
      throw new Error(`Failed to extract transactions from batch ${batchIdx + 1}: ${error}`);
    }
  }
